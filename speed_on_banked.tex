...
\usetikzlibrary{calc, arrows.meta, angles, quotes, decorations.markings, intersections}
...
We add the force vectors tip to tail as follows (using different parameters for better illustration):
...
\centering
    ...
    \centering
        \begin{tikzpicture}[
            ...
            \coordinate (O) at (0,0);
            \draw[mid arrow, cyan] (O) -- ++(0, -\Fg) coordinate (A) node[midway, right] {$\vec{F}_g=mg$};
            \draw[->, magenta, name path=lineFN] (A) -- ++({90+\incline}:\FN) coordinate (B) node[midway, above] {$\vec{F}_N$};
            \draw[mid arrow, magenta] (B) -- ++({180+\incline}:\Ff) coordinate (C) node[midway, left] {$\vec{F}_f$};
            \draw[mid arrow, dashed, name path=lineFNet] (O) -- (C) node[, midway, below left, font=\small] {$\sum\vec{F}$};


            \fill [name intersections={of=lineFN and lineFNet, by=P}] (P) circle (1.5pt) node[below] {$P$};

            \pic [draw, "$\theta$", angle radius=1.0cm] {angle = O--A--B};
            \pic [draw, "$\theta$", angle radius=0.8cm] {angle = O--C--B};
    ...
    \begin{minipage}{0.5\textwidth}
        \begin{tikzpicture}[
            ...
            \draw[mid arrow, magenta] (B) -- ++({180+\incline}:\Ff) coordinate (C) node[midway, right] {$\vec{F}_f$};
            \draw[->, dashed] (O) -- (C) node[midway, below] {$\sum\vec{F}$};


            \path[name path=extFN] (A) -- ($(A)!2!(B)$);
            \path[name path=extFNet] (O) -- ($(O)!2!(C)$);

            \path[name intersections={of=extFN and extFNet, by=P}];
            ...
            \pic [draw, "$\theta$", angle radius=1.2cm] {angle = O--A--B};
            \pic [draw, "$\theta$", angle radius=0.8cm] {angle = P--C--B};
            \pic [draw, angle radius=0.3cm, cyan] {right angle = C--O--A};
            \pic [draw, angle radius=0.3cm, magenta] {right angle = A--B--C};
...
For horizontal uniform circular motion we require $\sum\vec{F}=F_{cp}=m\left(\frac{v^2}{r}\right)$\textit{ horizontally in towards center of velodrome}. Hence, from the diagrams:
\begin{align*}
    \left| \sum\vec{F}\right| &= OP \pm PC\\
    &= (OA\tan\theta)\pm PC\qquad\because\triangle OAP \\
    \implies m \frac{v^2}{r} &= mg\tan\theta \pm PC \\ 
    \therefore PC &= \pm m\left(\frac{v^2}{r}-g\tan\theta\right) \tag{E1.1.1}
\end{align*}

This $PC$ in $\triangle PBC$ now gives us the vital constraint on how far $\vec{F}_N$ and $\vec{F}_f$ can extend, and so we can now solve for them explicitly:
\begin{alignat*}{2}
    |F_N| &= AP \pm PB   \hspace{5em}   &     |F_f| &= BC \\ 
    &= \left(\frac{OA}{\cos\theta}\right) \pm PC\sin\theta\quad\quad\because\triangle OAP, \triangle PBC \hspace{5em} &  &= PC \cos\theta \quad\because\triangle PBC \\
    &= \frac{mg}{\cos\theta}+m\left(\frac{v^2}{r}\sin\theta-g\frac{\sin^2\theta}{\cos\theta}\right)\quad\because\text{(E1.1.1)} & &=\pm m\left(\frac{v^2}{r}\cos\theta-g\sin\theta\right)\quad\because\text{(E1.1.1)} \\ 
    &= m\left(\frac{v^2}{r}\sin\theta+g\frac{1-\sin^2\theta}{\cos\theta}\right) & & \\ 
    &= m \left(\frac{v^2}{r}\sin\theta + g\cos\theta\right)\quad \because\sin^2\theta+\cos^2\theta=1
\end{alignat*}
...
\subsubsection{Longer Algebraic Approach}
...
\begin{align*}
    ...
    \vec{0}&=-mg \mp |F_f|\sin\theta + |F_N|\cos\theta \tag{E1.2.1}\\
    (\text{E1.2.1})\cdot \cos\theta\implies\qquad\qquad 
        \vec{0} &=-mg\cos\theta \mp |F_f|\sin\theta\cos\theta + |F_N|\cos^2\theta \tag{E1.2.2}\\
\end{align*}
...
\begin{align*}
    ...
    \vec{0}&= m\frac{v^2}{r} \mp |F_f|\cos\theta - |F_N|\sin\theta \tag{E1.2.3}\\
    (\text{E1.2.3})\cdot \sin\theta\implies\qquad\qquad 
        \vec{0}&= m\frac{v^2}{r}\sin\theta \mp |F_f|\sin\theta\cos\theta - |F_N|\sin^2\theta \tag{E1.2.4}\\
\end{align*}
...
\begin{align*}
    \text{(E1.2.4)} - \text{(E1.2.2)} \implies \qquad
        ...
        |F_N| &= m \left(\frac{v^2}{r} \sin\theta + g\cos\theta \right) \tag{E1.2.5}
\end{align*}


This gives us the normal force! Now, plugging into (E1.2.1), we find the friction force

\begin{align*}
    \text{(E1.2.5)}\rightarrow\text{(E1.2.1)}\implies\quad\quad
...
By noting that maximal static friction is proportional to the normal force, we can obtain the range of speeds explicitly.
...
\begin{align*}
    ...
    m\frac{v^2}{r}&=mg\cdot\tan(\theta\pm\alpha) \\ 
    \therefore |v| &= \sqrt{rg\cdot\tan(\theta\pm\alpha)}
\end{align*}

We now find the range of values for $\alpha$, which doesn't depend on the case so there is no $(\pm)$:
...
\begin{center}
\boxed{\sqrt{rg\cdot\tan(\theta-\arctan\mu)}\le|v|\le\sqrt{rg\cdot\tan(\theta+\arctan\mu)}}
\end{center}
...
This is significantly longer than the geometric approach and requires some more advanced knowledge to simplify the answer fully.
...
Simplification requires knowledge of how to sum sinusoidals with same frequency (which is $1$ here):

\begin{align*}
    \sqrt{rg\left(\frac{\sin\theta - \mu\cos\theta}{\cos\theta + \mu\sin\theta}\right)} &\le
        |v| \le
        \sqrt{rg\left(\frac{\sin\theta + \mu\cos\theta}{\cos\theta - \mu\sin\theta}\right)} \\
    \sqrt{rg\left(\frac{\left(\sqrt{1+\mu^2}\right)\sin(\theta-\arctan\mu)}{\left(\sqrt{1+\mu^2}\right)\cos(\theta-\arctan\mu)}\right)} &\le
        |v| \le
        \sqrt{rg\left(\frac{\left(\sqrt{1+\mu^2}\right)\sin(\theta+\arctan\mu)}{\left(\sqrt{1+\mu^2}\right)\cos(\theta+\arctan\mu)}\right)}
\end{align*}

\begin{center}$\therefore\;$\boxed{\sqrt{rg\cdot\tan(\theta-\arctan\mu)} \le
|v| \le
\sqrt{rg\cdot\tan(\theta+\arctan\mu)}
}\end{center}
